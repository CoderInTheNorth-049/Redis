...
static uint32_t next_timer_ms() {
    ...
    Conn *next = container_of(g_data.idle_list.next, Conn, idle_list); // Get the next connection in the idle list
    uint64_t next_us = next->idle_start + k_idle_timeout_ms * 1000; // Calculate the next timeout in microseconds
    if (next_us <= now_us) {
        // missed?
        return 0;
    }

    return (uint32_t)((next_us - now_us) / 1000); // Return the time until the next timeout in milliseconds
}
...
static void process_timers() {
    ...
    while (!dlist_empty(&g_data.idle_list)) {
        ...
        printf("removing idle connection: %d\n", next->fd); // Print the file descriptor of the connection being removed
        conn_done(next); // Remove and free the idle connection
    }
}
