...
static uint32_t do_get(const vector<string>& cmd, uint8_t* res, uint32_t* reslen) {
    if (!g_map.count(cmd[1])) return RES_NX; // Check if the key exists in g_map; if not, return RES_NX (resource not existing)

    string& val = g_map[cmd[1]]; // Get the value associated with the key
...
static uint32_t do_set(const vector<string>& cmd, uint8_t* res, uint32_t* reslen) {
    ...
    g_map[cmd[1]] = cmd[2]; // Set the value associated with the key (cmd[1]) to cmd[2] in g_map
    return RES_OK; // Return RES_OK to indicate success
}
...
static uint32_t do_del(const vector<string>& cmd, uint8_t* res, uint32_t* reslen) {
    (void)res; // Marking res as unused to avoid compiler warnings
    (void)reslen; // Marking reslen as unused to avoid compiler warnings
    g_map.erase(cmd[1]); // Remove the key-value pair associated with cmd[1] from g_map
    return RES_OK; // Return RES_OK to indicate success
}
