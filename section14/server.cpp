...
static void entry_destroy(Entry *ent) {
    switch (ent->type) {
    case T_ZSET:
        zset_dispose(ent->zset);  // Deallocate the zset structure
        delete ent->zset;  // Delete the pointer to the zset structure
        break;
    }
    delete ent;  // Delete the entry itself
}

// dispose the entry asynchronously
static void entry_del_async(void *arg) {
    entry_destroy((Entry *)arg);  // Call entry_destroy to deallocate the entry
}

// dispose the entry after it got detached from the key space
static void entry_del(Entry *ent) {
    entry_set_ttl(ent, -1);  // Set the TTL to -1 to indicate immediate expiration

    const size_t k_large_container_size = 10000;  // Threshold for large containers
    bool too_big = false;
    switch (ent->type) {
    case T_ZSET:
        too_big = hm_size(&ent->zset->hmap) > k_large_container_size;  // Check if the zset is too big
        break;
    }

    if (too_big) {
        thread_pool_queue(&g_data.tp, &entry_del_async, ent);  // Queue for asynchronous deletion if too big
    } else {
        entry_destroy(ent);  // Immediately destroy if not too big
    }
}
